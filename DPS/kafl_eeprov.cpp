...
#define ServiceName L"eeprov.dll"
#define HarnessName L"kafl_eeprov.dll"
...
int trigger() {
    ...
    //hprintf("size: %d\n", size);
    if (size >= 0x28) {
        hyper arg1 = payload_buffer->arg1;
        long arg3 = (long)((size - 8) / 2);
        long arg5 = (long)(size - 8 - arg3);

        wchar_t* arg2 = (wchar_t*)malloc((arg3 + 2) * sizeof(wchar_t));
        unsigned char* arg4 = (unsigned char*)malloc((arg5 + 1) * sizeof(unsigned char));

        int i, j;
        for (i = 0; i < arg3; ++i) {
            arg2[i] = (wchar_t)(payload_buffer->data[i]);
        }
        arg2[arg3] = 0;

        for (j = 0; j < arg5; ++j) {
            arg4[j] = (unsigned char)(payload_buffer->data[j+i]);
        }
        arg4[arg5] = 0;

        arg3 = arg3 * 2;
        status = Proc0_PtRpcSetProcessTag(hBinding, arg1, arg2, arg3, arg4, arg5);
    }

...
#define ServiceName L"tapisrv.dll"
#define HarnessName L"kafl_tapisrv.dll"
...
//    /* [length_is][size_is][out][in] */ unsigned char* arg_1,
//    /* [in] */ long arg_2,
//    /* [out][in] */ long* arg_3);
...
int trigger() {
    ...
    //hprintf("size: %d\n", size);
    if (size >= 24) {
        uint32_t data = *(uint32_t *)payload_buffer->data;
        if (size >= 60 && data < 0xA9 && data != 0) {
            long arg2 = size;
            long* arg3 = &arg2;
            unsigned char* arg1 = &(payload_buffer->data[0]);

            Proc1_ClientRequest(hBinding, chandle, arg1, arg2, arg3);
        }
        else if(data == 0){
            long arg2 = size;
            long* arg3 = &arg2;

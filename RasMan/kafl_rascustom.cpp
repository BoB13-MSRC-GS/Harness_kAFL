...
#define ServiceName L"rascustom.dll"
#define HarnessName L"kafl_rascustom.dll"
...
bool set_ip_range2(HMODULE hModule, HMODULE hModule2, byte idx) {
    ...
    WORD i;
    uint64_t codeStart, codeStart2;
    uint64_t codeEnd, codeEnd2;
    ...
    for (i = 0; i < pNtHeaders->FileHeader.NumberOfSections; ++i) {
        ...
        // Check for the .text section
        if (memcmp((LPVOID)pSectionHeader->Name, PE_CODE_SECTION_NAME, strlen(PE_CODE_SECTION_NAME)) == 0) {
            codeStart = (uint64_t)hModule + pSectionHeader->VirtualAddress;
            codeEnd = codeStart + pSectionHeader->Misc.VirtualSize;
    ...
    // Get the PE header of the current module.
    PIMAGE_NT_HEADERS pNtHeaders2 = (PIMAGE_NT_HEADERS)(
        (PBYTE)hModule2 + ((PIMAGE_DOS_HEADER)hModule2)->e_lfanew);
    if (pNtHeaders2->Signature != IMAGE_NT_SIGNATURE) {
        hprintf("[Harness][set_ip_range] Invalid PE signature\n");
        habort("Invalid PE signature\n");
        return false;
    }
    hprintf("\taddr: %p\n", (char*)hModule2);
    // Get the section headers.
    PIMAGE_SECTION_HEADER pSectionHeaders2 = (PIMAGE_SECTION_HEADER)((PBYTE)pNtHeaders2 +
        sizeof(IMAGE_NT_HEADERS));
    for (i = 0; i < pNtHeaders2->FileHeader.NumberOfSections; ++i) {
        PIMAGE_SECTION_HEADER pSectionHeader2 = &pSectionHeaders2[i];

        // Check for the .text section
        if (memcmp((LPVOID)pSectionHeader2->Name, PE_CODE_SECTION_NAME, strlen(PE_CODE_SECTION_NAME)) == 0) {
            codeStart2 = (uint64_t)hModule2 + pSectionHeader2->VirtualAddress;
            codeEnd2 = codeStart2 + pSectionHeader2->Misc.VirtualSize;
            ...
            buffer[0] = codeStart < codeStart2 ? codeStart : codeStart2; // low range
            buffer[1] = codeEnd > codeEnd2 ? codeEnd : codeEnd2; // high range
            buffer[2] = idx; // IP filter index [0-3]
            kAFL_hypercall(HYPERCALL_KAFL_RANGE_SUBMIT, (uint64_t)buffer);
...
void submit_ip_ranges() {
    ...
    wchar_t subDLL[] = L"eappcfg.dll";

    hModule = GetModuleHandleW(ServiceName);
    hModule2 = GetModuleHandleW(subDLL);
    hprintf("[Harness][submit_ip_ranges] Call set_ip_range - %ws, %ws\n", ServiceName, subDLL);
    if (!set_ip_range2(hModule, hModule2, idx)) {
        hprintf("[Harness][submit_ip_ranges] Failed set_ip_range - %ws, %ws\n", ServiceName, subDLL);
        return;
    }
    hprintf("[Harness][submit_ip_ranges] Success set_ip_range - %ws, %ws\n", ServiceName, subDLL);
...
int trigger() {
    ...
    wchar_t* arg1 = (wchar_t*)malloc(0x1000 * sizeof(wchar_t));
    struct Struct_36_t* arg3 = (struct Struct_36_t*)malloc(sizeof(struct Struct_36_t));
    INT64 test = 0;
    void* chandle = &test;
    Proc0_Rpc_VpnProtEngOpen(hBinding, arg1, &chandle, arg3);
    ...
    if (size > 4) {
        long arg_3 = payload_buffer->arg3;
        long arg_2 = size - 4;
        byte* arg_1 = &(payload_buffer->data[0]);
        wchar_t* arg_4 = nullptr;
        Proc35_Rpc_VpnProtEngWinRtGetEapXml(hBinding, arg_1, arg_2, arg_3, &arg_4);
    }

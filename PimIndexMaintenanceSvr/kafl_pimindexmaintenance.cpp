...
#include <dbghelp.h>
#include <psapi.h>
#include "PimIndexMaintenance.h"

#define ServiceName L"PimIndexMaintenance.dll"
#define HarnessName L"kafl_pimindexmaintenance.dll"
...
typedef struct {
    int32_t size;
    long StructMember0[3];
    long StructMember1[3];
    long StructMember2[3];
    long StructMember3[3];
    long arg2;
    byte data[];
} kAFL_custom;
#pragma pack(pop)
...
int trigger() {
    ...
    INT64 test = 0;
    void* chandle = &test;
    status = Proc7_PimIMService_CreateIndexedFilter(hBinding, 1, 1, &chandle);
    ...
    //hprintf("size: %d\n", size);
    if (size >= 53) {
        Struct_34_t* arg1 = new Struct_34_t;
        arg1->StructMember0 = size - 48; // real  =  size - 52
        arg1->StructMember1 = &(payload_buffer->data[0]);
        long arg2 = payload_buffer->arg2;;
        long arg3 = 4;
        //Struct_60_t* arg4 = (Struct_60_t*)malloc(4 * sizeof(Struct_60_t));
        Struct_60_t* arg4 = (Struct_60_t*)&(payload_buffer->StructMember0[0]);

        status = Proc8_PimIMService_SetStaticFilter(hBinding, chandle, arg1, arg2, arg3, arg4);
    }
